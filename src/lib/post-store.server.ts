...
async function ensureD1Seeds(db: D1Database): Promise<void> {
  for (const s of CHAPTER_POST_SEEDS) {
    const existing = await db
      .prepare("SELECT slug, body_markdown, published FROM posts WHERE slug = ?")
      .bind(s.slug)
      .first<{ slug: string; body_markdown: string; published: number }>();
    if (!existing) {
      await db
        .prepare(
          `INSERT INTO posts (slug, title, summary, ingress, thumbnail, body_markdown, published, created_at, updated_at)
    ...
    if (s.slug === "platetektonikk" && isShortPlatetektonikkBody(existing.body_markdown ?? "")) {
      await db
        .prepare(
